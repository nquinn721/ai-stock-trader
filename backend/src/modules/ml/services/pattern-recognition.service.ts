...
export class PatternRecognitionService {
  ...
  async recognizePatterns(
    ...
    this.logger.log(`Recognizing patterns for ${symbol} across ${timeframes.length} timeframes`);
  ...
  private async detectTemporalPatterns(
    ...
    const meanReversionPatterns = await this.detectMeanReversionPatterns(data, timeframe);
    patterns.push(...meanReversionPatterns);

    // Seasonal patterns
    const seasonalPatterns = await this.detectSeasonalPatterns(data, timeframe, symbol);
    patterns.push(...seasonalPatterns);

    // Volume-price divergence patterns
    const divergencePatterns = await this.detectDivergencePatterns(data, timeframe);
  ...
  private async detectHeadAndShoulders(data: any[], timeframe: string): Promise<any | null> {
    const highs = this.findLocalMaxima(data.map(d => d.high), 10);
    ...
    for (let i = 1; i < highs.length - 1; i++) {
      ...
      if (head.value > leftShoulder.value && head.value > rightShoulder.value) {
        ...
        if (shoulderDiff < 0.05) { // 5% tolerance
          ...
          const necklineSupport = this.calculateNecklineSupport(data, leftShoulder.index, rightShoulder.index);
          
          return {
            type: 'head_and_shoulders',
            confidence: this.calculateHeadShouldersConfidence(leftShoulder, head, rightShoulder, necklineSupport),
            timeframe,
            ...
            targetPrice: necklineSupport.level - (head.value - necklineSupport.level),
            keyLevels: {
  ...
  private async detectDoubleTops(data: any[], timeframe: string): Promise<any[]> {
    const patterns: any[] = [];
    const highs = this.findLocalMaxima(data.map(d => d.high), 8);
    ...
    for (let i = 0; i < highs.length - 1; i++) {
      for (let j = i + 1; j < highs.length; j++) {
        ...
        if (priceDiff < 0.03 && timeDiff > 10 && timeDiff < 100) { // 3% price tolerance
          // Find valley between peaks
          const valleyIndex = this.findLowestPoint(data, first.index, second.index);
          const valley = data[valleyIndex];
          
          // Check if valley is significantly lower
          const valleyDrop = (first.value - valley.low) / first.value;
          
          if (valleyDrop > 0.05) { // 5% minimum drop
            patterns.push({
              type: 'double_top',
              confidence: this.calculateDoubleTopConfidence(first, second, valley, timeDiff),
              timeframe,
  ...
  private async detectTriangles(data: any[], timeframe: string): Promise<any[]> {
    const patterns: any[] = [];
  ...
  private async detectFlagsAndPennants(data: any[], timeframe: string): Promise<any[]> {
    const patterns: any[] = [];
    ...
    for (let i = 10; i < data.length - 10; i++) {
      ...
      const poleGain = (data[poleEnd].high - data[poleStart].low) / data[poleStart].low;
      
      if (poleGain > 0.1) { // 10% minimum gain for pole
        ...
        if (flagVolatility < 0.02) { // Low volatility during consolidation
          patterns.push({
            type: 'bull_flag',
            ...
            targetPrice: data[poleEnd].high + (data[poleEnd].high - data[poleStart].low),
            keyLevels: {
  ...
  private async detectCupAndHandle(data: any[], timeframe: string): Promise<any | null> {
    ...
    if (leftDepth > 0.15 && leftDepth < 0.5 && rightHeight > 0.9 && rightHeight < 1.1) {
      ...
      if (handleVolatility < 0.05) {
        return {
          type: 'cup_and_handle',
          confidence: this.calculateCupHandleConfidence(leftDepth, rightHeight, handleVolatility),
          timeframe,
  ...
  private async detectMomentumPatterns(data: any[], timeframe: string): Promise<any[]> {
    const patterns: any[] = [];
  ...
  private async detectMeanReversionPatterns(data: any[], timeframe: string): Promise<any[]> {
    const patterns: any[] = [];
    
    // Calculate moving averages
    const ma20 = this.calculateSMA(data.map(d => d.close), 20);
    const ma50 = this.calculateSMA(data.map(d => d.close), 50);
    ...
    if (currentPrice < currentMA20 * 0.95 && currentPrice < currentMA50 * 0.97) {
      patterns.push({
        type: 'oversold_mean_reversion',
        confidence: Math.min(1, (currentMA20 - currentPrice) / currentPrice * 10),
        timeframe,
    ...
    if (currentPrice > currentMA20 * 1.05 && currentPrice > currentMA50 * 1.03) {
      patterns.push({
        type: 'overbought_mean_reversion',
        confidence: Math.min(1, (currentPrice - currentMA20) / currentPrice * 10),
        timeframe,
        startDate: new Date(data[data.length - 10].timestamp),
        endDate: new Date(data[data.length - 1].timestamp),
        prediction: 'BEARISH' as const,
  ...
  private async detectSeasonalPatterns(data: any[], timeframe: string, symbol: string): Promise<any[]> {
    const patterns: any[] = [];
    ...
    if (timeframe === '1d') {
      ...
      if (currentMonthPerf && Math.abs(currentMonthPerf) > 0.02) {
        patterns.push({
          ...
          endDate: new Date(data[data.length - 1].timestamp),
          prediction: currentMonthPerf > 0 ? 'BULLISH' as const : 'BEARISH' as const,
        });
      }
    }
  ...
  private async detectDivergencePatterns(data: any[], timeframe: string): Promise<any[]> {
    const patterns: any[] = [];
  ...
  private async detectGartleyPattern(data: any[], timeframe: string): Promise<any | null> {
    ...
    for (let i = 0; i < swings.length - 4; i++) {
      ...
      if (this.isInRange(abRatio, 0.618, 0.05) &&
          this.isInRange(bcRatio, 0.382, 0.05) &&
          this.isInRange(cdRatio, 1.272, 0.1)) {
        
        return {
          type: 'gartley',
          confidence: this.calculateHarmonicConfidence(abRatio, bcRatio, cdRatio),
          timeframe,
          startDate: new Date(data[X.index].timestamp),
          endDate: new Date(data[D.index].timestamp),
          prediction: D.value > C.value ? 'BEARISH' as const : 'BULLISH' as const,
          keyLevels: {
            X: X.value,
            A: A.value,
            B: B.value,
  ...
  private async detectButterflyPattern(data: any[], timeframe: string): Promise<any | null> {
  ...
  private async detectBatPattern(data: any[], timeframe: string): Promise<any | null> {
    // Bat pattern has specific Fibonacci relationships
    // Implementation would be similar to Gartley with bat-specific ratios
    return null; // Simplified for now
  }

  private async detectCrabPattern(data: any[], timeframe: string): Promise<any | null> {
  ...
  private async detectImpulseWaves(data: any[], timeframe: string): Promise<any[]> {
    const patterns: any[] = [];
    const swings = this.findSwingPoints(data);
    ...
    for (let i = 0; i < swings.length - 4; i++) {
      ...
      if (this.validateImpulseWaveStructure(waves)) {
        patterns.push({
          ...
          prediction: waves[4].value > waves[0].value ? 'BULLISH' as const : 'BEARISH' as const,
          waveCount: 5,
        });
      }
    }
  ...
  private async detectCorrectiveWaves(data: any[], timeframe: string): Promise<any[]> {
    const patterns: any[] = [];
    const swings = this.findSwingPoints(data);
  ...
  private async fusePatterns(patterns: any[]): Promise<any[]> {
    ...
    for (let i = 0; i < patterns.length; i++) {
      if (processed.has(i)) continue;      const pattern = patterns[i];
      ...
      if (overlapping.length > 0) {
        ...
        const combinedPattern = this.combinePatterns(pattern, overlapping.map(idx => patterns[idx]));
  ...
  private async adjustForMarketContext(patterns: any[], symbol: string): Promise<any[]> {
    // Adjust pattern predictions based on current market context
    const marketContext = await this.getMarketContext();
    
    return patterns.map(pattern => ({
      ...pattern,
      confidence: this.adjustConfidenceForMarket(pattern.confidence, pattern.prediction, marketContext),
  ...
  private findLocalMaxima(values: number[], windowSize: number): Array<{ index: number; value: number }> {
  ...
  private findLocalMinima(values: number[], windowSize: number): Array<{ index: number; value: number }> {
  ...
  private findLowestPoint(data: any[], startIndex: number, endIndex: number): number {
  ...
  private calculateNecklineSupport(data: any[], leftIndex: number, rightIndex: number): { level: number; strength: number } {
    ...
    const touches = lows.filter(low => Math.abs(low - level) / level < 0.02).length;
  ...
  private calculateSMA(values: number[], period: number): number[] {
    ...
    for (let i = period - 1; i < values.length; i++) {
      const sum = values.slice(i - period + 1, i + 1).reduce((sum, v) => sum + v, 0);
      sma.push(sum / period);
    }
  ...
  private calculateMonthlyPerformance(data: any[]): number[] {
    ...
    return monthlyPerf.map((perf, i) => monthlyCounts[i] > 0 ? perf / monthlyCounts[i] : 0);
  }

  private findSwingPoints(data: any[]): Array<{ index: number; value: number; type: 'high' | 'low' }> {
    const swings: Array<{ index: number; value: number; type: 'high' | 'low' }> = [];
    const windowSize = 5;
    
    // Find swing highs
    const highs = this.findLocalMaxima(data.map(d => d.high), windowSize);
    highs.forEach(h => swings.push({ ...h, type: 'high' }));
    
    // Find swing lows
    const lows = this.findLocalMinima(data.map(d => d.low), windowSize);
  ...
  private calculateHeadShouldersConfidence(leftShoulder: any, head: any, rightShoulder: any, neckline: any): number {
    const symmetry = 1 - Math.abs(leftShoulder.value - rightShoulder.value) / leftShoulder.value;
    const headHeight = (head.value - neckline.level) / neckline.level;
    const necklineStrength = neckline.strength;
    
    return Math.min(1, (symmetry * 0.4 + Math.min(headHeight * 2, 1) * 0.4 + necklineStrength * 0.2));
  }

  private calculateDoubleTopConfidence(first: any, second: any, valley: any, timeDiff: number): number {
    ...
    return Math.min(1, (symmetry * 0.5 + valleyDepth * 2 * 0.3 + timeScore * 0.2));
  }

  private calculateFlagConfidence(poleGain: number, flagVolatility: number): number {
  ...
  private calculateCupHandleConfidence(depth: number, heightRatio: number, handleVolatility: number): number {
  ...
  private calculateHarmonicConfidence(abRatio: number, bcRatio: number, cdRatio: number): number {
  ...
  private detectSymmetricalTriangle(data: any[], timeframe: string): any | null {
  ...
  private adjustConfidenceForMarket(confidence: number, prediction: string, marketContext: any): number {
    let adjustment = 1.0;
  ...
  private generateContextualNote(pattern: any, marketContext: any): string {
    ...
    } else if (pattern.prediction !== 'NEUTRAL' && marketContext.trend !== 'NEUTRAL') {
  ...
  private async logPatternRecognition(
    ...
    try {
      const prediction = this.mlPredictionRepository.create({
        ...
        confidence: result.patterns.length > 0 
      ...
      this.logger.warn(`Failed to log pattern recognition for ${symbol}:`, error);
    }
  }
