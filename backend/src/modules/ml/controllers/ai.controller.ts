...
export class AIController {
  ...
  async explainRecommendation(
    @Body() recommendationDto: ExplainRecommendationDto,
    ...
    const explanation =
      await this.explainableAIService.explainRecommendation(recommendation);
    return { explanation };
  }
  ...
  async getConversationHistory(
    ...
    return this.tradingAssistantService.getConversationHistory(
      userId,
      conversationId,
      limit,
    );
  ...
  async createNewConversation(@Param('userId') userId: string) {
    const conversationId =
      await this.tradingAssistantService.createNewConversation(userId);
  ...
  @ApiQuery({
    name: 'limit',
    description: 'Number of explanations to return',
    required: false,
  })
  @ApiResponse({
    status: 200,
  ...
  async getMarketCommentary(@Body() body: { symbols?: string[] }) {
  ...
  async getPortfolioInsights(
    ...
    return this.tradingAssistantService.getPortfolioInsights(
      body.userId,
      body.portfolioId,
    );
  }
  ...
  async getTradingStrategySuggestions(
    ...
    return this.tradingAssistantService.generateStrategySupggestions(
      userProfile,
    );
  }
