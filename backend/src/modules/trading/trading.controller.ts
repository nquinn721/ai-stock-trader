...
export class TradingController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Active trading signals retrieved successfully',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Stock trading signals retrieved successfully',
  })
  async getSignalsForStock(
    @Param('symbol') symbol: string,
    @Query('limit') limit: string = '10',
  ) {
    return await this.tradingService.getSignalsForStock(
      symbol,
      parseInt(limit),
    );
  }
  ...
  @ApiResponse({
    status: 200,
    description: 'Breakout analysis retrieved successfully',
  })
