...
import { BacktestResult, PerformanceMetrics, RiskMetrics, TradeDetail } from './entities/backtest-result.entity';
...
class BacktestEngine {
  ...
  constructor(params: { initialCapital: number; commission: number; slippage: number }) {
  ...
  private async enterPosition(signal: Signal, price: number, timestamp: Date): Promise<void> {
    ...
    if (totalCost > this.portfolio.cash) {
      ...
      const maxAffordable = Math.floor((this.portfolio.cash - commissionCost) / price);
    ...
    if (existingPosition) {
      ...
      const totalCost = (existingPosition.quantity * existingPosition.entryPrice) + (signal.size * price);
  ...
  private async exitPosition(signal: Signal, price: number, timestamp: Date): Promise<void> {
  ...
  async updatePortfolio(marketData: MarketData): Promise<void> {
    ...
    if (position) {
      ...
      position.unrealizedPnL = (marketData.close - position.entryPrice) * position.quantity;
    }
    ...
    const drawdown = (this.highWaterMark - this.portfolio.totalValue) / this.highWaterMark;
...
export class BacktestingService {
  ...
  async runBacktest(strategy: TradingStrategy, params: BacktestParams): Promise<BacktestResult> {
    try {
      this.logger.log(`Starting backtest for strategy ${strategy.id}: ${strategy.name}`);
      ...
      // Get historical data
      const historicalData = await this.getHistoricalData(params.symbols, params.startDate, params.endDate);
      ...
      for (const dataPoint of historicalData) {
        ...
        const signals = await this.generateSignals(strategy, dataPoint, historicalData.slice(0, dataIndex + 1));
      ...
      const performanceMetrics = this.calculatePerformanceMetrics(portfolio, params.initialCapital);
      const riskMetrics = this.calculateRiskMetrics(portfolio, params.initialCapital);
      ...
      this.logger.log(`Backtest completed for strategy ${strategy.id} in ${backtestResult.executionTimeMs}ms`);
      return backtestResult;

    } catch (error) {
      this.logger.error(`Backtest failed for strategy ${strategy.id}: ${error.message}`);
      
      // Update status to failed
      await this.backtestRepository.update({ strategyId: strategy.id }, {
        status: 'failed',
  ...
  async generateSignals(strategy: TradingStrategy, marketData: MarketData, historicalData: MarketData[]): Promise<Signal[]> {
      ...
      if (component.type === 'condition' && component.category === 'entry') {
        ...
        if (shouldEnter) {
          ...
          signals.push({
            ...
            metadata: { componentId: component.id, componentName: component.name }
          });
        }
      }
    }
  ...
  private async evaluateCondition(component: any, context: any): Promise<boolean> {
    ...
    switch (component.name.toLowerCase()) {
      ...
      case 'rsi oversold':
        const rsi = this.calculateRSI(context.historical, component.parameters.period || 14);
        return rsi < (component.parameters.threshold || 30);
      
      case 'rsi overbought':
        const rsiOverbought = this.calculateRSI(context.historical, component.parameters.period || 14);
  ...
  private calculatePositionSize(strategy: TradingStrategy, context: any): number {
    // Find position sizing rule
    const positionSizeRule = strategy.riskRules?.find(rule => rule.type === 'position_size');
    ...
    switch (positionSizeRule.parameters.method) {
      ...
      case 'percentage':
        ...
        return Math.floor((positionSizeRule.parameters.percentage || 0.05) * 10000 / context.currentPrice);
  ...
  private calculatePerformanceMetrics(portfolio: Portfolio, initialCapital: number): PerformanceMetrics {
    ...
    const returns = portfolio.equityCurve.map((point, index) => {
      if (index === 0) return 0;
      return (point.value - portfolio.equityCurve[index - 1].value) / portfolio.equityCurve[index - 1].value;
    }).slice(1);
    
    const avgReturn = returns.reduce((sum, r) => sum + r, 0) / returns.length;
    const returnVariance = returns.reduce((sum, r) => sum + Math.pow(r - avgReturn, 2), 0) / returns.length;
    const sharpeRatio = returnVariance > 0 ? (avgReturn * Math.sqrt(252)) / Math.sqrt(returnVariance * 252) : 0;
  ...
  private calculateRiskMetrics(portfolio: Portfolio, initialCapital: number): RiskMetrics {
  ...
  async getBacktestResult(backtestId: string): Promise<BacktestResult> {
    const result = await this.backtestRepository.findOne({ where: { id: backtestId } });
    if (!result) {
