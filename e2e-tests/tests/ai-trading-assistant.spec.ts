...
test.describe("AI Trading Assistant", () => {
  ...
  test("should display AI assistant button and open chat", async ({ page }) => {
    ...
    await expect(
      page.getByText(/Welcome to your AI Trading Assistant/)
    ).toBeVisible();
  });
...
test.describe("AI Explanations", () => {
  ...
  test("should display explanation components when available", async ({
    page,
  }) => {
    ...
    console.log(
      "AI explanation feature test - to be fully implemented when backend integration is complete"
    );
  });
});
