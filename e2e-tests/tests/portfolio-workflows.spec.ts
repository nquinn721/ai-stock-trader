...
test.describe("Portfolio Management Workflows", () => {
  ...
  test("should display portfolio overview", async ({ page }) => {
    ...
    const portfolioSection = page.locator(".portfolio, .portfolio-summary, [data-testid='portfolio']");
    ...
    if (await portfolioSection.isVisible()) {
      ...
      if (await emptyState.isVisible()) {
        await expect(emptyState).toContainText(/No.*position|Empty.*portfolio|No.*holdings/i);
      }
    }
  });
  ...
  test("should display position details", async ({ page }) => {
    ...
    if (await positions.count() > 0) {
      ...
      const positionDetails = firstPosition.locator(".quantity, .shares, .value, .price");
  ...
  test("should show portfolio performance metrics", async ({ page }) => {
    ...
    const performanceElements = page.locator(".performance, .pnl, .gain-loss, .profit-loss");
  ...
  test("should handle portfolio actions", async ({ page }) => {
    ...
    const actionButtons = page.locator("button").filter({ hasText: /Buy|Sell|Trade/i });
  ...
  test("should display portfolio charts when available", async ({ page }) => {
    ...
    const charts = page.locator(".chart, .recharts-wrapper, canvas, svg[class*='chart']");
  ...
  test("should show transaction history", async ({ page }) => {
    ...
    if (await historySection.isVisible()) {
      ...
      const transactions = historySection.locator(".transaction, .trade, .history-item");
      ...
      if (await transactions.count() > 0) {
        ...
        // Empty history state
        await expect(historySection).toContainText(/No.*transaction|Empty.*history/i);
      }
    }
  });
  ...
  test("should calculate portfolio totals correctly", async ({ page }) => {
    ...
    const totalValue = page.locator(".total-value, .portfolio-value, .net-worth");
  ...
  test("should handle empty portfolio state", async ({ page }) => {
    ...
    const emptyState = page.locator(".empty-state, .no-positions, .empty-portfolio");
    
    if (await emptyState.isVisible()) {
      await expect(emptyState).toBeVisible();
      await expect(emptyState).toContainText(/No.*position|Start.*trading|Empty/i);
      
      // Should provide action to start trading
      const startTradingButton = emptyState.locator("button, a").filter({ hasText: /Start|Trade|Buy/i });
  ...
  test("should sort and filter positions", async ({ page }) => {
    ...
    if (await positions.count() > 1) {
      ...
      const sortButton = page.locator("button, select").filter({ hasText: /Sort|Order/i });
