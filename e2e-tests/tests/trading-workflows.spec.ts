...
test.describe("Stock Trading Workflows", () => {
  ...
  test("should browse stock data effectively", async ({ page }) => {
    ...
    if ((await stockCards.count()) > 0) {
      ...
      await expect(stocksContainer).toContainText(
        /No.*stock.*data|Loading|unavailable/i
      );
    }
  });
  ...
  test("should navigate portfolio section", async ({ page }) => {
    ...
    const portfolioLink = page
      .locator("text=Portfolio, a[href*='portfolio']")
      .first();
    ...
    if (await portfolioLink.isVisible()) {
      ...
      const portfolioContent = page.locator(
        ".portfolio-summary, .position, .no-portfolio"
      );
  ...
  test("should display performance charts when available", async ({ page }) => {
    ...
    const chartContainers = page.locator(
      ".chart, .recharts-wrapper, canvas, svg"
    );
    ...
    if ((await chartContainers.count()) > 0) {
      ...
      const noChartMessage = page.locator(
        "text=/No.*chart|No.*data|Chart.*unavailable/i"
      );
  ...
  test("should handle stock search functionality", async ({ page }) => {
    ...
    const searchInput = page
      .locator("input[placeholder*='search'], input[placeholder*='stock']")
      .first();
  ...
  test("should display real-time connection status", async ({ page }) => {
    ...
    const connectionIndicator = page.locator(
      ".connection-status, .websocket-status, .status"
    );

    if (await connectionIndicator.isVisible()) {
      await expect(connectionIndicator).toContainText(
        /Connected|Connecting|Disconnected/i
      );
    }

    // Check for real-time data updates indicator
    const liveDataIndicator = page.locator(
      ".live-data, .updating, .last-updated"
    );
