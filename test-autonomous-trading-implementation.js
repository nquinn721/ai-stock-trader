...
console.log('🚀 Starting Autonomous Trading Agent Builder (S40) Integration Tests');
console.log('=' * 80);
...
async function testBackendServices() {
  ...
  try {
    ...
    try {
      ...
      console.log('  ⚠️  Backend health check failed (expected if server not running)');
    }
...
function testFrontendComponents() {
  ...
  try {
    ...
    const dashboardPath = path.join(__dirname, 'frontend/src/components/autonomous-trading/AutonomousAgentDashboard.tsx');
    const apiServicePath = path.join(__dirname, 'frontend/src/services/autonomousTradingApi.ts');
    ...
    if (fs.existsSync(dashboardPath)) {
      ...
      if (dashboardContent.includes('handleStrategyAction') && dashboardContent.includes('handleDeployStrategy')) {
...
function generateTestReport() {
  ...
  const backendScore = Object.values(testResults.backendTests).filter(Boolean).length;
  const frontendScore = Object.values(testResults.frontendTests).filter(Boolean).length;
  const integrationScore = Object.values(testResults.integrationTests).filter(Boolean).length;
