...
export const NotificationProvider: React.FC<NotificationProviderProps> = ({
  ...
  const bulkDismiss = useCallback(
    async (notificationIds: number[]) => {
      try {
        const result = await notificationService.bulkDismiss(
          notificationIds,
          userId
        );
        if (result.successful > 0) {
          ...
          const unreadCount = notifications.filter(
            (n) => notificationIds.includes(n.id) && n.status === "sent"
          ).length;
  ...
  const updatePreference = useCallback(
    async (
      ...
      try {
        const success = await notificationService.updatePreference(
          userId,
          type,
          preference
        );
        if (success) {
          setPreferences((prev) => {
            ...
            if (existing) {
              return prev.map((p) =>
                p.type === type ? { ...p, ...preference } : p
              );
            } else {
              return [
                ...prev,
                { userId, type, ...preference } as NotificationPreference,
              ];
            }
          });
        }
    ...
    (filter: Partial<NotificationFilter>) => {
      return notifications.filter((notification) => {
        ...
        if (filter.priority && notification.priority !== filter.priority)
          return false;
        if (filter.symbol && notification.symbol !== filter.symbol)
          return false;
        if (
          filter.portfolioId &&
          notification.portfolioId !== filter.portfolioId
        )
          return false;
        ...
        if (
          filter.fromDate &&
          new Date(notification.createdAt) < filter.fromDate
        )
          return false;
        if (filter.toDate && new Date(notification.createdAt) > filter.toDate)
          return false;
  ...
  const showBrowserNotification = useCallback(
    (notification: Notification, isSystemAlert = false) => {
      ...
      if ("Notification" in window && Notification.permission === "granted") {
        const options = {
          ...
          requireInteraction:
            isSystemAlert || notification.priority === "critical",
