...
class MacroIntelligenceService {
  ...
  async getEconomicAnalysis(country: string): Promise<EconomicAnalysis> {
    const response = await axios.get(`${this.baseURL}/economic/analysis/${country}`);
  ...
  async getGDPForecast(country: string) {
    const response = await axios.get(`${this.baseURL}/gdp-forecast?country=${country}`);
    return response.data;
  }
  ...
  async getRecessionProbability(country: string) {
    const response = await axios.get(`${this.baseURL}/recession-probability?country=${country}`);
    return response.data;
  }
  ...
  async getQEProbability(centralBank: string) {
    const response = await axios.get(`${this.baseURL}/monetary-policy/qe-assessment?centralBank=${centralBank}`);
    return response.data;
  }
  ...
  async getPoliticalStability(country: string) {
    const response = await axios.get(`${this.baseURL}/geopolitical/political-stability?country=${country}`);
    return response.data;
  }
  ...
  async getDiplomaticTensions(region: string) {
    const response = await axios.get(`${this.baseURL}/geopolitical/diplomatic-tensions?region=${region}`);
    return response.data;
  }
  ...
  async getMacroDashboard(country: string): Promise<MacroDashboardData> {
    const response = await axios.get(`${this.baseURL}/comprehensive-analysis?country=${country}`);
    return response.data;
  }
  ...
  async getGlobalOverview(): Promise<GlobalOverviewData> {
    const response = await axios.get(`${this.baseURL}/comprehensive-analysis?country=GLOBAL`);
    return response.data;
  }
  ...
  async getSystemHealth() {
    const response = await axios.get(`${this.baseURL}/comprehensive-analysis?country=US`);
    return response.data;
  }
  ...
  async getDashboardData(country: string): Promise<{
    ...
    try {
      ...
      const monetary: MonetaryPolicyAnalysis = {
        centralBank: country === 'US' ? 'Federal Reserve' : `${country} Central Bank`,
