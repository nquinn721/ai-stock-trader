...
class AutonomousTradingApi {
  private baseURL = `${API_BASE_URL}/autonomous-trading`;
  ...
  async getRunningStrategies(
    ...
    try {
      const response = await axios.get(
        `${this.baseURL}/running?userId=${userId}`
      );
      return {
        success: true,
        data: response.data
  ...
  async getAvailableStrategies(
    ...
    try {
      const response = await axios.get(
        `${this.baseURL}/marketplace?userId=${userId}`
      );
      return {
        success: true,
        data: response.data
  ...
  async getComponentLibrary(): Promise<ApiResponse<any[]>> {
    try {
