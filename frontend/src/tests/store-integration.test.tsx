...
describe('Store Integration Tests', () => {
  ...
  describe('StockStore Integration', () => {
    ...
    it('should update stocks from WebSocket data', async () => {
      ...
      expect(stockStore.stocks[0].currentPrice).toBe(152.50);
      expect(stockStore.stocks[0].changePercent).toBe(2.70);
    });
  });
  ...
  describe('ApiStore Integration', () => {
    ...
    it('should handle network errors', async () => {
      ...
      await expect(apiStore.get('/test-endpoint')).rejects.toThrow('Network Error');
    });
  });
  ...
  describe('Cross-Store Integration', () => {
    it('should update portfolio when position prices change', async () => {
      ...
      const updatedPosition = portfolioStore.positions.find(p => p.symbol === 'AAPL');
    ...
    it('should handle concurrent API calls without race conditions', async () => {
      ...
      jest.spyOn(apiStore, 'get')
        .mockImplementationOnce(() => 
          new Promise(resolve => setTimeout(() => resolve({ id: 1, positions: [] }), 100))
        )
        .mockImplementationOnce(() => 
          new Promise(resolve => setTimeout(() => resolve([{ symbol: 'AAPL' }]), 50))
        );
