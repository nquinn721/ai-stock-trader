...
const Dashboard: React.FC = () => {
  ...
  const fetchTradingSignals = async () => {
    try {
      const response = await axios.get('http://localhost:3000/trading/signals');
  ...
  const getSignalForStock = (stockSymbol: string) => {
    return signals.find(signal => 
      signal.stock?.symbol === stockSymbol && signal.isActive
    );
  };

  const analyzeStock = async (symbol: string) => {
    try {
      await axios.post(`http://localhost:3000/trading/analyze/${symbol}`);
  ...
  const analyzeAllStocks = async () => {
    try {
      setLoading(true);
      await axios.post('http://localhost:3000/trading/analyze-all');
      ...
      <header className="dashboard-header">
        ...
        <div className="header-info">
          ...
          <div className="stats">
            ...
            <span>Active Signals: {signals.filter(s => s.isActive).length}</span>
          </div>
        </div>
      </header>
        ...
        <div className="signals-grid">
          {tradingSignals.slice(0, 3).map((signal, index) => (
            <div key={index} className="signal-summary">
              <div className="signal-stock">{signal.stock?.symbol || 'Unknown'}</div>
