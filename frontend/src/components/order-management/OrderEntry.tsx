...
export const OrderEntry: React.FC<OrderEntryProps> = observer(
  ({ symbol, portfolioId, currentPrice = 0, onOrderSubmitted, onClose }) => {
    ...
    const [profitTargetPrice, setProfitTargetPrice] = useState<number>(
      currentPrice * 1.1
    );
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        const orderData = {
          ...
          trailAmount:
            orderType === OrderType.TRAILING_STOP ? trailAmount : undefined,
          trailPercent:
            orderType === OrderType.TRAILING_STOP ? trailPercent : undefined,
          profitTargetPrice:
            orderType === OrderType.BRACKET ? profitTargetPrice : undefined,
          stopLossPrice:
            orderType === OrderType.BRACKET ? stopLossPrice : undefined,
          conditionalTriggers:
            conditionalTriggers.length > 0 ? conditionalTriggers : undefined,
        };
    ...
    const updateConditionalTrigger = (
      ...
      setConditionalTriggers(
        conditionalTriggers.map((t) => (t.id === id ? { ...t, ...updates } : t))
      );
    };

    const estimatedCost =
      side === OrderSide.BUY ? quantity * (limitPrice || currentPrice) : 0;
    const estimatedProceeds =
      side === OrderSide.SELL ? quantity * (limitPrice || currentPrice) : 0;
          ...
          <div className="form-row">
            ...
            <div className="form-group">
              ...
              <div className="side-buttons">
                <button
                  type="button"
                  className={`side-button buy ${
                    side === OrderSide.BUY ? "active" : ""
                  }`}
                ...
                <button
                  type="button"
                  className={`side-button sell ${
                    side === OrderSide.SELL ? "active" : ""
                  }`}
                  ...
                  {conditionalTriggers.map((trigger, index) => (
                    <div key={trigger.id} className="trigger-row">
                      <select
                        value={trigger.type}
                        onChange={(e) =>
                          updateConditionalTrigger(trigger.id, {
