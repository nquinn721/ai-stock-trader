...
export const OrderManagement: React.FC<OrderManagementProps> = observer(({
  ...
  const selectAllOrders = () => {
    const activeOrderIds = filteredOrders
      .filter(order => order.status === 'pending' || order.status === 'triggered')
      .map(order => order.id);
  ...
  return (
    <div className="order-management">
        ...
        {showPortfolioFilter && (
          <div className="portfolio-selector">
            <select
              ...
              onChange={(e) => setSelectedPortfolio(Number(e.target.value) || undefined)}
      ...
      <div className="order-tabs">
        ...
        <button
          className={`tab-button ${activeTab === 'conditional' ? 'active' : ''}`}
      ...
      <div className="order-filters">
        ...
        {activeTab === 'active' && selectedOrders.size > 0 && (
          <div className="bulk-actions">
            ...
            <button onClick={handleCancelSelected} className="bulk-cancel-btn">
              Cancel Selected
            </button>
                      ...
                      <tr>
                        <th>
                          <input
                            ...
                            onChange={filteredOrders.length > 0 && selectedOrders.size === filteredOrders.length ? clearSelection : selectAllOrders}
                          />
                        </th>
                        <th>Symbol</th>
                        <th>Type</th>
                        ...
                        <tr key={order.id} className={selectedOrders.has(order.id) ? 'selected' : ''}>
                          ...
                          <td>
                            <div className="order-type">
                              ...
                              {order.ocoGroupId && <span className="oco-badge">OCO</span>}
                              {order.trailAmount && <span className="trail-badge">TRAIL</span>}
                            </div>
                          </td>
                          <td>
                          ...
                          <td>
                            <div className="price-info">
                              {order.limitPrice && <div>Limit: {formatCurrency(order.limitPrice)}</div>}
                              {order.stopPrice && <div>Stop: {formatCurrency(order.stopPrice)}</div>}
                              {order.trailAmount && <div>Trail: ${order.trailAmount}</div>}
                              {order.trailPercent && <div>Trail: {order.trailPercent}%</div>}
                            </div>
                          </td>
                          <td>
                          ...
                          <td>
                            <div className="order-actions">
                              ...
                              {order.ocoGroupId && (
                                <button
                                  onClick={() => handleCancelOCOGroup(order.ocoGroupId!)}
                        ...
                        <tr key={order.id}>
                          ...
                          <td>
                            ...
                            {order.fillCount && order.fillCount > 1 && (
                              <span className="fill-count">({order.fillCount} fills)</span>
                            )}
                          </td>
                          <td>
                            {order.limitPrice ? formatCurrency(order.limitPrice) : 
                             order.stopPrice ? formatCurrency(order.stopPrice) : 'Market'}
                          </td>
                          <td>
                        ...
                        <tr key={order.id}>
                          ...
                          <td>
                            <div className="triggers">
                              {order.conditionalTriggers?.map((trigger, index) => (
                                <div key={trigger.id} className="trigger">
                                  ...
                                  <span className="trigger-condition">
                                    {trigger.field} {trigger.condition} {trigger.value}
                                  </span>
                                </div>
                              ))}
                            </div>
                  ...
                  <table>
                    ...
                    <tbody>
                      {filteredOrders
                        ...
                        .slice(0, 50)
                        .map(order => {
                          const slippage = order.limitPrice && order.executedPrice ?
                            Math.abs(order.executedPrice - order.limitPrice) / order.executedPrice * 100 : 0;
                          ...
                          return (
                            <tr key={order.id}>
                              ...
                              <td>{order.executedQuantity || order.quantity}</td>
                              <td>
                                {order.limitPrice ? formatCurrency(order.limitPrice) : 'Market'}
                              </td>
                              <td>{formatCurrency(order.executedPrice)}</td>
                              <td>{slippage > 0 ? `${slippage.toFixed(3)}%` : '-'}</td>
                              <td>
                                {order.executedAt && order.createdAt ?
                                  `${((new Date(order.executedAt).getTime() - new Date(order.createdAt).getTime()) / 1000).toFixed(2)}s` :
                                  '-'
                                }
                              </td>
                              <td>{formatCurrency(order.commission)}</td>
