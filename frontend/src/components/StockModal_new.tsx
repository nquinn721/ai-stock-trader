...
const StockModal: React.FC<StockModalProps> = ({ stock, isOpen, onClose }) => {
  ...
  const isPositive = changePercent >= 0; // Generate chart data for Material-UI chart
  const generateChartData = () => {
    ...
    // Generate 24 hours of data with more realistic price movements
    let currentPrice = basePrice * 0.99; // Start slightly below current price

    for (let i = 0; i < 24; i++) {
      const timeHour = (now.getHours() - 23 + i) % 24;

      // Create more realistic price movements
      const priceChange = (Math.random() - 0.5) * volatility * basePrice * 0.5;
      currentPrice = Math.max(currentPrice + priceChange, basePrice * 0.85);

      // Ensure final price approaches current price
      if (i === 23) {
        currentPrice = basePrice;
      } else if (i > 18) {
        // Gradually move towards current price in last few hours
        const targetDrift = (basePrice - currentPrice) * 0.3;
        currentPrice += targetDrift;
      }

      data.push({
        time: `${timeHour.toString().padStart(2, "0")}:00`,
                ...
                <ResponsiveContainer width="100%" height={280}>
                  ...
                  >
                    ...
                    <XAxis
                      ...
                      stroke="#8ba3f7"
                      fontSize={11}
                      tick={{ fill: "#8ba3f7", fontSize: 11 }}
                      axisLine={{ stroke: "#8ba3f7" }}
                    ...
                    <Tooltip
                      ...
                      labelStyle={{ color: "#8ba3f7", fontWeight: "bold" }}
                      formatter={(value: any) => [
              ...
              >
                <div className="toggle-header">
                  ...
                  <div className="section-summary">
                    ...
                    <span className="summary-text">
                      Vol: +{(Math.random() * 50 + 20).toFixed(0)}%
                    </span>
                  </div>
                  <span
                    className={`chevron ${
            ...
            <div className="expandable-section">
              ...
              >
                <div className="toggle-header">
                  ...
                  <div className="section-summary">
                    <span className="sentiment-badge positive">BULLISH</span>
                    <span className="summary-text">
                      RSI: {stock.breakoutStrategy?.rsi?.toFixed(0) || "--"}
                    </span>
                  </div>
                  <span
                    className={`chevron ${
