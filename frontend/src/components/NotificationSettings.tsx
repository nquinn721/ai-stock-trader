...
const NotificationSettings: React.FC<NotificationSettingsProps> = ({
  ...
  const getPreference = (type: NotificationType): NotificationPreference => {
    return (
      localPreferences.find((p) => p.type === type) || {
        userId: "default_user",
        type,
        enabled: true,
            ...
            <FormControlLabel
              control={
                <Switch
                  ...
                  onChange={(e) =>
                    updateLocalPreference(type, { enabled: e.target.checked })
                  }
                />
              }
              label="Enabled"
                    ...
                    <FormGroup row>
                      {Object.values(DeliveryChannel).map((channel) => (
                        <FormControlLabel
                          ...
                          control={
                            <Switch
                              checked={preference.deliveryChannels.includes(
                                channel
                              )}
                              onChange={(e) => {
                                const channels = e.target.checked
                                  ? [...preference.deliveryChannels, channel]
                                  : preference.deliveryChannels.filter(
                                      (c) => c !== channel
                                    );
                                updateLocalPreference(type, {
                ...
                {type === NotificationType.TRADING_OPPORTUNITY && (
                  <Box sx={{ mb: 3 }}>
                    <Typography variant="body2" gutterBottom>
                      ...
                      {preference.customThresholds?.minConfidence || 70}%
                    </Typography>
                    <Slider
                ...
                {type === NotificationType.TECHNICAL_ALERT && (
                  <Box sx={{ mb: 3 }}>
                    ...
                    <Box display="flex" flexWrap="wrap" gap={1}>
                      {["RSI", "MACD", "Bollinger Bands", "Volume"].map(
                        (indicator) => (
                          <Chip
                            key={indicator}
                            label={indicator}
                            clickable
                            color={
                              preference.customThresholds?.indicators?.includes(
                                indicator
                              )
                                ? "primary"
                                : "default"
                            ...
                            onClick={() => {
                              const indicators =
                                preference.customThresholds?.indicators || [];
                              const updated = indicators.includes(indicator)
                                ? indicators.filter(
                                    (i: string) => i !== indicator
                                  )
  ...
  const renderQuietHoursSettings = () => {
    const globalPreference = getPreference(
      NotificationType.TRADING_OPPORTUNITY
    );

    return (
      <Box>
        <Alert severity="info" sx={{ mb: 3 }}>
          Quiet hours apply to all non-critical notifications. Critical alerts
        ...
        <Card>
          <CardContent>
            <Box display="flex" alignItems="center" mb={2}>
              ...
              <Box ml="auto">
                <FormControlLabel
                  control={
                    <Switch
                      checked={globalPreference.quietHours?.enabled || false}
                      onChange={(e) => {
                        // Update all preferences with quiet hours setting
                        Object.values(NotificationType).forEach((type) => {
                          const currentQuietHours = getPreference(type)
                            .quietHours || {
                            enabled: false,
                            start: "22:00",
                            end: "08:00",
            ...
            {globalPreference.quietHours?.enabled && (
              <Box
                sx={{
                  display: "flex",
                  gap: 3,
      ...
      <DialogContent>
        ...
        <TabPanel value={activeTab} index={0}>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
            Configure which types of notifications you want to receive and how
        ...
        <TabPanel value={activeTab} index={2}>
          <Alert severity="info" sx={{ mb: 2 }}>
            Browser notifications require permission. Click the button below to
            enable them.
          </Alert>
          <Button
            variant="outlined"
