...
export const stories: Story[] = [`;
    ...
    const formattedStories = stories.map(story => {
      ...
      const dependencies = story.dependencies.length > 0 
   ...
   */
  static saveStoriesFile(stories: Story[], filename: string = 'stories.ts'): void {
    try {
      ...
      throw new Error(`Failed to save stories file: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  static saveStoriesAsJSON(stories: Story[], filename: string = 'stories-backup.json'): void {
    try {
      ...
      throw new Error(`Failed to save stories JSON: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  static createBackupFilename(prefix: string = 'stories', extension: string = 'ts'): string {
