...
async function testRecommendationPipeline() {
  ...
  try {
    ...
    const historyResponse = await axios.get(`${baseUrl}/stocks/AAPL/history?period=1mo`);
    console.log(`✅ History data retrieved: ${historyResponse.data.length} data points`);
    
    // 3. Test recommendation generation
    console.log('\n3. Testing recommendation generation...');
    try {
      const recommendationResponse = await axios.post(`${baseUrl}/recommendation-pipeline/generate`, {
        symbols: ['AAPL'],
        targetPortfolios: [1],
