...
async function testStepByStep() {
  try {
    ...
    console.log(
      ...
      stockResponse.data.symbol,
      stockResponse.data.currentPrice
    );
    ...
    console.log(
      "✅ ML enhanced recommendation works - Action:",
      mlRecResponse.data.action
    );

    // 4. Test pipeline config (should work)
    console.log("\n4. Testing pipeline config...");
    const configResponse = await axios.get(
      "http://localhost:8000/api/recommendation-pipeline/config"
    );
