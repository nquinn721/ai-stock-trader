...
const sqlFilePath = path.join(
  ...
  "..",
  "database",
  "cleanup-forex-crypto.sql"
);

// Check for a SQL query passed via command-line argument
const sqlQuery = process.argv
  .find((arg) => arg.startsWith("--sql="))
  ?.split("=")[1];
...
async function executeSql() {
  ...
  try {
    ...
    console.log(
      `Connecting to database "${connectionConfig.database}" on ${connectionConfig.host}...`
    );
